...
const Edit = ({ token }) => {
    ...
    return (
        <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} className='w-full max-w-[500px] px-3 py-2' type="text" required />

            {/* Description */}
            <p className='mt-2 mb-2'>Product Description</p>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} className='w-full max-w-[500px] px-3 py-2' required />

            {/* Category, SubCategory, Price */}
            <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
                <div>
                    <p className='mb-2 mt-1'>Product Category</p>
                    <select value={category} onChange={(e) => setCategory(e.target.value)} className='w-full px-2 py-3'>
                ...
                <div>
                    <p className='mb-2 mt-1'>Product SubCategory</p>
                    <select value={subCategory} onChange={(e) => setSubCategory(e.target.value)} className='w-full px-2 py-3'>
                ...
                <div>
                    ...
                    <input value={price} onChange={(e) => setPrice(e.target.value)} className='w-full px-2 py-3 sm:w-[120px]' type="number" />
                </div>
            </div>
            ...
            {/* Submit */}
            <button type="submit" className='w-28 py-3 mt-4 bg-black text-white cursor-pointer'>UPDATE</button>
        </form>
    )
}
